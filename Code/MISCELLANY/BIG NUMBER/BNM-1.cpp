...
struct Bint
{
    ...
    // -1 if |x| < |y|
    // 1 if |x| > |y|
    friend int __compare_abs(const Bint& x, const Bint& y)
    {
    ...
    // Note: sign ignored.
    void __internal_add(const Bint& v)
    {
        if (a.size() < v.a.size())
        {
            a.resize(v.a.size(), 0);
    ...
    // Note: sign ignored.
    void __internal_sub(const Bint& v)
    {
    ...
    std::is_convertible<L, Bint>::value &&
    std::is_convertible<R, Bint>::value &&
    std::is_lvalue_reference<R&&>::value,
        Bint>::type friend operator + (L&& l, R&& r)
    {
        Bint result(std::forward<L>(l));
        result += r;
        return result;
    }
    template< typename L, typename R >
    ...
    Bint mul_simple(const Bint &v) const
    {
        Bint res;
    ...
    Bint mul_karatsuba(const Bint &v) const
    {
    ...
    Bint mul_fft(const Bint& v) const
    {
        Bint res;
        res.sign = sign * v.sign;
    ...
    friend Bint sqrt(const Bint &a1)
    {
        ...
        Bint r = (long long) a.a[n - 1] * BASE + a.a[n - 2];
        ...
        for(int j = n / 2 - 1; j >= 0; j--)
        {
            for(; ; --q)
            {
                Bint r1 = (r - (res * 2 * Bint(BASE) + q) * q) * Bint(BASE) * Bint(BASE) + (j > 0 ? (long long) a.a[2 * j - 1] * BASE + a.a[2 * j - 2] : 0);
                if (r1 >= 0)
                {
                    r = r1;
                    break;
