// TAB_TRIGGER "__NUMBER_THEORETIC_TRANSFORM"
...
vector<ll> convolve(vector<ll> x, vector<ll> y, ll mod)
{
    for(auto &a : x)a %= mod;
    for(auto &b : y)b %= mod;
...
vector<ll> parse(const string &s)
{
    ...
    {
        int b = n - WIDTH * i, x = 0;
        for(int a = max(0ll, b - WIDTH) ; a < b ; a++)
        {
            x = x * 10 + s[a] - '0';
        }
        v[i] = x;
...
string parse(const vector<ll> &v)
{
    string s;
    int i, N = v.size();
    vector<ll> digits(N + 1, 0);
    for(i = 0 ; i < N ; i++)digits[i] = v[i];
    ll c = 0;
    for(i = 0 ; i < N ; i++)
    {
        c += digits[i];
